...
def reg_user(id,ref):
    ...
    if sql.fetchone() is None:
        sql.execute(f"INSERT INTO trafik VALUES (?,?)", ('channel1', 't.me/test12321312312'))
        db.commit()
...
def cheak_linkss():
    db = sqlite3.connect('server.db')
    sql = db.cursor()
    i1 = sql.execute(f"SELECT parametr FROM trafik WHERE chanel = 'channel1'").fetchone()[0]
    print(i1)
    return i1
