...
import sqlite3
from .sqlit import info_members, obnovatrafika1,cheak_linkss
...
async def admin_ka(message: types.Message):
    ...
    if id in ADMIN_ID:
        ...
        bat_z = types.InlineKeyboardButton(text='Настроить канал залива', callback_data='settings_ch')
...
@dp.callback_query_handler(text='settings_ch')  #НАСТРОИКА КАНАЛА ЗАЛИВА ТРАФИКА
async def checkfdfds(call: types.callback_query,state: FSMContext):
    if call.message.chat.id in ADMIN_ID:
        await bot.send_message(call.message.chat.id, f"Введите новую ссылку для пиара ({cheak_linkss()})")
        await get_Fdfds.step1.set()
    await bot.answer_callback_query(call.id)

@dp.message_handler(state=get_Fdfds.step1,content_types='text')  # Получили новую ссылку
async def redarkt_post3424(message: types.Message, state: FSMContext):
    obnovatrafika1(message.text)
